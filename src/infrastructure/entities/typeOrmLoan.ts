...
export class TypeORMLoan {
  ...
  @ManyToOne(() => TypeORMBook) // * A Loan can be formed by several books, while a Book can exist in multiple active loans while there is available stock.
  @JoinColumn() // * JoinTable() is used only in ManyToMany relations. It's used here in Loan because it holds the ForeignKey
  book: TypeORMBook;
